...
def generate_test_data(params):
    ...
    if num_topk > 0:
        ...
        _, pos_to_token, _, token_topk_to_pos, _, _, _, _ = (
            fly_tile_kernels.moe.get_fused_mapping(topk_idx, num_experts, 0, 128)
        )
    ...
    if is_fused_cast_back:
        x = fly_tile_kernels.quant.per_token_cast(x, 'e4m3', num_per_channels)

    func = lambda: fly_tile_kernels.quant.per_channel_cast_fused(
