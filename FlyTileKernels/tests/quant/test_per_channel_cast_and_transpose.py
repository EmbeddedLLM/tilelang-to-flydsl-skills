...
def test_per_channel_cast_and_transpose(params):
    ...
    x_fp8, x_sf = fly_tile_kernels.quant.per_channel_cast_and_transpose(x, 'e4m3', num_per_channels, round_sf)
    x_fp8_ref, x_sf_ref = fly_tile_kernels.torch.cast(x, 'e4m3', block_size=(num_per_channels, 1), round_sf=round_sf)
...
def test_per_channel_cast_and_transpose_benchmark(benchmark_timer, benchmark_record, params):
    ...
    t_us = benchmark_timer(
        lambda: fly_tile_kernels.quant.per_channel_cast_and_transpose(x, 'e4m3', num_per_channels, round_sf)
    )
