...
def generate_test_data(params):
    ...
    _, pos_to_token, pos_to_token_topk, token_topk_to_pos, _, _, _, _ = fly_tile_kernels.moe.get_fused_mapping(
        topk_idx, num_experts, 0, num_per_channels
    )
    x_expand = fly_tile_kernels.moe.expand_to_fused(x, token_topk_to_pos, pos_to_token)
    x = fly_tile_kernels.quant.per_token_cast(x_expand, 'e4m3', num_per_channels=num_per_channels)
...
def test_swiglu_backward_and_per_token_cast(params):
    ...
    func = lambda: fly_tile_kernels.quant.swiglu_backward_and_per_token_cast(
        x,
        weighted_act_x_grad,
        topk_weights,
        pos_to_token_topk,
    ...
    def func_ref():
        out, x_grad_full, weight_grad = fly_tile_kernels.torch.swiglu_backward(
            x,
            weighted_act_x_grad,
            topk_weights,
            pos_to_token_topk,
        ...
        x_grad_fp8_full, x_grad_fp8_sf = fly_tile_kernels.torch.cast(
