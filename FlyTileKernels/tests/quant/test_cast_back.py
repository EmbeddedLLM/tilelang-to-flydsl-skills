...
from fly_fly_tile_kernels.testing.numeric import assert_equal, calc_diff, count_bytes
...
def generate_test_data_per_token(params):
    ...
    x_fp8, x_sf = fly_tile_kernels.quant.per_token_cast(
    ...
    func = lambda: fly_tile_kernels.quant.per_token_cast_back((x_fp8, x_sf), out_dtype_str, num_per_channels=num_per_channels)
...
def generate_test_data(params):
    ...
    x = torch.randn((num_tokens, hidden), dtype=out_dtype, device='cuda')
    x_casted, x_sf = fly_tile_kernels.torch.cast(x, fmt, (num_per_tokens, num_per_channels), round_sf=round_sf)
    out_dtype_str = dtype_to_str(out_dtype)
    func = lambda: fly_tile_kernels.quant.cast_back(
...
def test_cast_back_per_token(params):
    ...
    x_fp8_bf16 = func()
    x_fp8_bf16_ref = fly_tile_kernels.torch.cast_back((x_fp8, x_sf), out_dtype_str, (1, num_per_channels))
...
def test_cast_back(params):
    ...
    x_casted_back = func()
    x_casted_back_ref = fly_tile_kernels.torch.cast_back((x_casted, x_sf), out_dtype_str, (num_per_tokens, num_per_channels))
