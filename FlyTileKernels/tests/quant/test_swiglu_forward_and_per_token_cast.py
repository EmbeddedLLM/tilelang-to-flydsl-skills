...
def generate_test_data(params):
    ...
    alignment = 16
    pos_to_expert, pos_to_token, pos_to_token_topk, token_topk_to_pos, _, _, _, _ = fly_tile_kernels.moe.get_fused_mapping(topk_idx, num_experts, 0, alignment)
    ...
    x = fly_tile_kernels.moe.expand_to_fused(x, token_topk_to_pos, pos_to_token)
...
def test_swiglu_forward_and_per_token_cast(params):
    ...
    for num_sms in generate_num_sms():
        ...
        x_fp8, x_sf = fly_tile_kernels.quant.swiglu_forward_and_per_token_cast(
...
def test_swiglu_forward_and_per_token_cast_benchmark(benchmark_timer, benchmark_record, params):
    ...
    func = lambda: fly_tile_kernels.quant.swiglu_forward_and_per_token_cast(
        **kernel_args, clamped_count=clamped_count
    )
    x_fp8, x_sf = func()
