...
def test_per_channel_cast(params):
    ...
    x_fp8, per_channel_sf_inv = fly_tile_kernels.quant.per_channel_cast(x, 'e4m3', num_per_tokens, round_sf)
    x_fp8_ref, per_channel_sf_inv_ref = fly_tile_kernels.torch.cast(x, 'e4m3', block_size=(num_per_tokens, 1), round_sf=round_sf)

    assert_equal(x_fp8, x_fp8_ref)
    assert_equal(per_channel_sf_inv, per_channel_sf_inv_ref)

    # Check bias
    x_casted_back = fly_tile_kernels.torch.cast_back((x_fp8_ref, per_channel_sf_inv_ref), 'fp32', (num_per_tokens, 1))
...
def test_per_channel_cast_benchmark(benchmark_timer, benchmark_record, params):
    ...
    t_us = benchmark_timer(lambda: fly_tile_kernels.quant.per_channel_cast(x, 'e4m3', num_per_tokens, round_sf))
