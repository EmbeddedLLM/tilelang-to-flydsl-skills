...
def test_per_token_cast(params):
    ...
    func = lambda: fly_tile_kernels.quant.per_token_cast(
        **base_args,
        num_per_channels=num_per_channels,
    )
    func_ref = lambda: fly_tile_kernels.torch.cast(
        **base_args,
        block_size=(1, num_per_channels),
    )
    x_casted, x_sf = func()
    x_casted_ref, x_sf_ref = func_ref()
    x_casted_back = fly_tile_kernels.torch.cast_back((x_casted, x_sf), 'fp32', (1, num_per_channels))
    ...
    if not in_with_sf_factor and fmt == 'e4m3' and not use_packed_ue8m0 and num_tokens > 0:
        ...
        x_non_contiguous_casted, x_non_contiguous_sf = fly_tile_kernels.quant.per_token_cast(
    ...
    if not in_with_sf_factor and num_per_channels != hidden:
        ...
        if not use_tma_aligned_col_major_sf:
            ...
            x_casted = fly_tile_kernels.quant.per_token_cast_with_precomputed_sf(
                **base_args, num_per_channels=num_per_channels, sf=x_sf
            )
            x_casted_ref = fly_tile_kernels.torch.cast(**base_args, block_size=(1, num_per_channels), sf=x_sf)
            assert_equal(x_casted, x_casted_ref)

        # Test sf only mode
        twice_sf_inv = fly_tile_kernels.quant.per_token_cast_with_sf_only(**base_args, num_per_channels=num_per_channels)
        if use_packed_ue8m0:
