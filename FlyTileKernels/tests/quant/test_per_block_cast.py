...
from fly_fly_tile_kernels.testing.quant import clear_unused_sf
...
def test_per_block_cast(params):
    ...
    # Test cast
    x_casted, per_block_sf_inv = fly_tile_kernels.quant.per_block_cast(**base_args)
    x_casted_ref, per_block_sf_inv_ref = fly_tile_kernels.torch.cast(**base_args)
    x_casted_back = fly_tile_kernels.torch.cast_back((x_casted, per_block_sf_inv), 'fp32', block_size)
    if use_packed_ue8m0:
    ...
    if not use_tma_aligned_col_major_sf:
        ...
        x_casted = fly_tile_kernels.quant.per_block_cast_with_precomputed_sf(**base_args, sf=per_block_sf_inv)
        x_casted_ref = fly_tile_kernels.torch.cast(**base_args, sf=per_block_sf_inv)
        assert_equal(x_casted, x_casted_ref)

    # Test sf only mode
    twice_per_block_sf_inv = fly_tile_kernels.quant.per_block_cast_with_sf_only(**base_args)
    if use_packed_ue8m0:
...
def test_per_block_cast_benchmark(benchmark_timer, benchmark_record, params):
    ...
    x_casted, per_block_sf_inv = fly_tile_kernels.quant.per_block_cast(**args)

    t_us = benchmark_timer(lambda: fly_tile_kernels.quant.per_block_cast(**args))
