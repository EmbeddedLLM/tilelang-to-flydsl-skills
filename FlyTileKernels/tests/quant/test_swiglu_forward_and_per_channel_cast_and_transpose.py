...
def test_swiglu_forward_and_per_channel_cast_and_transpose(params):
    ...
    func = lambda: fly_tile_kernels.quant.swiglu_forward_and_per_channel_cast_and_transpose(
        x,
        'e4m3',
        num_per_tokens=num_per_tokens,
        round_sf=round_sf,
    ...
    def func_ref():
        act_out = fly_tile_kernels.torch.swiglu_forward(
            x,
            swiglu_clamp_value=swiglu_clamp_value,
        ).bfloat16()
        x_fp8_ref, x_sf_ref = fly_tile_kernels.torch.cast(
            act_out,
            'e4m3',
