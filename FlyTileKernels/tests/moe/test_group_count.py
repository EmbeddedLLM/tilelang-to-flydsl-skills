...
from fly_fly_tile_kernels.torch import group_count as torch_group_count
...
def test_group_count(params):
    ...
    for num_sms in generate_num_sms():
        set_num_sms(num_sms)
        count = fly_tile_kernels.moe.group_count(topk_idx, num_experts)
...
def test_group_count_benchmark(benchmark_timer, benchmark_record, params):
    ...
    t_us = benchmark_timer(lambda: fly_tile_kernels.moe.group_count(topk_idx, num_experts))
