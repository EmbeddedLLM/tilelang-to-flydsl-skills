...
from fly_fly_tile_kernels.testing.numeric import calc_diff, count_bytes
from fly_fly_tile_kernels.torch import aux_fi as torch_aux_fi
...
def test_aux_fi(params):
    ...
    for num_sms in generate_num_sms():
        set_num_sms(num_sms)
        fi = fly_tile_kernels.moe.aux_fi(topk_idx, num_experts, num_aux_topk)
...
def test_aux_fi_benchmark(benchmark_timer, benchmark_record, params):
    ...
    t_us = benchmark_timer(lambda: fly_tile_kernels.moe.aux_fi(topk_idx, num_experts, num_aux_topk))
