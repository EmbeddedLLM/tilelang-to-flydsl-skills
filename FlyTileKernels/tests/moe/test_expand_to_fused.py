...
def generate_test_data_expand_to_fused(params):
    ...
    pos_to_expert, _, _, token_topk_to_pos, _, _, _, _ = fly_tile_kernels.moe.get_fused_mapping(topk_idx, num_experts, 0, 16)
...
def test_expand_to_fused(params):
    ...
    expanded_x = fly_tile_kernels.moe.expand_to_fused(x, token_topk_to_pos, pos_to_expert)

    # Test correctness: torch reference
    expanded_x_ref = fly_tile_kernels.torch.expand_to_fused(x, token_topk_to_pos, pos_to_expert)
...
def test_expand_to_fused_benchmark(benchmark_timer, benchmark_record, params):
    ...
    t_us = benchmark_timer(lambda: fly_tile_kernels.moe.expand_to_fused(x, token_topk_to_pos, pos_to_expert))
...
def test_expand_to_fused_with_sf(params):
    ...
    func = lambda: fly_tile_kernels.moe.expand_to_fused_with_sf(
    ...
    # Test correctness: torch reference
    expanded_x_ref, expanded_x_sf_ref = fly_tile_kernels.torch.expand_to_fused_with_sf(
        (x_fp8, x_sf.contiguous()), num_per_channels, token_topk_to_pos, pos_to_expert, use_tma_aligned_col_major_sf,
    )
    assert_equal(expanded_x, expanded_x_ref)
