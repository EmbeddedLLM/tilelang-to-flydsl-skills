...
def test_transpose(params):
    ...
    y = fly_tile_kernels.transpose.transpose(x)
    if num_tokens == 0:
        return
...
def test_transpose_benchmark(benchmark_timer, benchmark_record, params):
    ...
    num_bytes = count_bytes(x, fly_tile_kernels.transpose.transpose(x))
    t_us = benchmark_timer(lambda: fly_tile_kernels.transpose.transpose(x))
...
def test_batched_transpose(params):
    ...
    y = fly_tile_kernels.transpose.batched_transpose(x)
...
def test_batched_transpose_benchmark(benchmark_timer, benchmark_record, params):
    ...
    num_bytes = count_bytes(x, fly_tile_kernels.transpose.batched_transpose(x))
    t_us = benchmark_timer(lambda: fly_tile_kernels.transpose.batched_transpose(x))
